...
export default async function PlanilhasProntasPage({ params }: PageProps) {
  ...
  const mapasDeCores: Record<string, { linha: string; botao: string; badge: string }> = {
    blue: { 
      linha: "bg-blue-500", 
  ...
  // 🔥 3. MAPEAMENTO TOTALMENTE DINÂMICO: Imagens e Links conectados ao seu Supabase
  const produtos = [
    {
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-36 pb-20 pointer-events-auto">
        <div className="text-center mb-12">
          <h1 className="text-3xl font-black text-neutral-950 uppercase tracking-wider">
            {ehEstetica ? "Nossos Produtos" : "Planilhas Prontas"}
          </h1>
          ...
          {produtos.map((prod, index) => (
            <div key={index} className="bg-white border border-neutral-200 rounded-2xl p-4 shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow relative z-20">
              ...
              <div className="relative w-full aspect-[4/3] rounded-xl overflow-hidden mb-4 bg-neutral-100">
                {/* 🔥 renderização dinâmica da imagem do banco */}
              ...
              <div>
                ...
                <h3 className="text-sm font-bold text-neutral-800 tracking-tight leading-snug mb-3 min-h-[40px]">
                  {prod.title}
                </h3>
              ...
              <div>
                ...
                <a 
                  ...
                  className={`relative z-30 block w-full text-center py-3 rounded-xl font-bold text-xs uppercase tracking-wider transition-all cursor-pointer pointer-events-auto select-none ${estiloAtivo.botao}`}
                >
