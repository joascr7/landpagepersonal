...
export default async function LojaPage({ params }: PageProps) {
  ...
  // Trata os preços vindos do banco para garantir que nunca quebrem a string com NaN
  ...
  const servicos = ehEstetica ? [
    {
      ...
      price: preco180,
      description: "Unhas impecáveis o ano todo com horários fixos e spa incluso.",
      badge: "VIP",
      checkoutUrl: `/${slugAtivo}#agendamento`, // Redireciona para o calendário nativo
      isExternal: false,
    },
    {
      ...
      badge: "Popular",
      checkoutUrl: `/${slugAtivo}#agendamento`,
      isExternal: false,
    },
    {
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {servicos.map((item, idx) => (
              <div key={idx} className="bg-white border border-neutral-200 rounded-2xl p-6 shadow-sm relative flex flex-col justify-between text-center z-20">
                {/* 🔥 Corrigido o fundo da badge para seguir o tema dinâmico */}
                ...
                <div>
                  ...
                  <h3 className="text-lg font-extrabold text-neutral-900 mt-1 min-h-[50px] flex items-center justify-center uppercase tracking-tight">{item.name}</h3>
                  ...
                  <p className="text-xs text-neutral-500 mt-2 min-h-[40px] leading-relaxed">{item.description}</p>
                </div>
                ...
                <a 
                  ...
                  className={`relative z-30 mt-6 block w-full text-center ${estiloAtivo.botao} font-black py-3 px-4 rounded-xl text-xs uppercase transition-all cursor-pointer pointer-events-auto select-none tracking-wider`}
                >
                  {ehEstetica ? "Agendar este Pacote" : "Adicionar ao carrinho"}
                </a>
              </div>
            ))}
          </div>
        ...
        <div id="planilhas" className="scroll-mt-28">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {produtosDigitais.map((item, idx) => (
              <div key={idx} className="bg-white border border-neutral-200 rounded-2xl overflow-hidden shadow-sm p-4 flex flex-col justify-between relative z-20 hover:shadow-md transition-shadow">
                {/* 🔥 Corrigido o fundo da badge secundária também para seguir o tema */}
                <span className={`absolute top-6 right-6 ${estiloAtivo.bgBadge} text-white text-[10px] font-black px-2.5 py-0.5 rounded-full uppercase z-30`}>
                  {item.badge}
                </span>
                <div>
                  <div className="h-40 w-full rounded-xl overflow-hidden relative mb-4 bg-neutral-100 border border-neutral-100">
                    <img src={item.img} alt={item.name} className="w-full h-full object-cover" />
                  </div>
                  <p className={`text-[11px] ${estiloAtivo.textBadge} font-bold uppercase tracking-wider`}>{item.category}</p>
                  <h3 className="text-xs font-bold text-neutral-800 mt-1 line-clamp-3 min-h-[48px] leading-relaxed">{item.name}</h3>
                  <div className="flex items-center gap-0.5 my-1">
                    {"★★★★★".split("").map((s, i) => <span key={i} className="text-amber-400 text-[10px]">★</span>)}
                  </div>
                  <div className="mt-3 flex items-center gap-2">
                    <span className="text-xs text-neutral-400 line-through font-medium">{item.oldPrice}</span>
                ...
                <a 
                  ...
                  className={`relative z-30 mt-4 block w-full text-center ${estiloAtivo.botao} font-black py-3 px-4 rounded-xl text-xs uppercase transition-all cursor-pointer pointer-events-auto select-none tracking-wider`}
                >
