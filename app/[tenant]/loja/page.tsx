...
export default async function LojaPage({ params }: PageProps) {
  ...
  const corBotao = ehEstetica ? "bg-pink-600 hover:bg-pink-700" : "bg-orange-600 hover:bg-orange-700";
  ...
  // Vitrine 2: Infoprodutos / Planilhas Dinâmicas
  const produtosDigitais = ehEstetica ? [
    {
      ...
      checkoutUrl: cliente.checkout_planilhas, // Usando a coluna de infoproduto
    },
    {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-20">
          <div className="text-center mb-12">
            ...
            <div className={`w-12 h-1 ${corLinha} mx-auto mt-2`}></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {servicos.map((item, idx) => (
              <div key={idx} className="bg-white border border-neutral-200 rounded-2xl p-6 shadow-sm relative flex flex-col justify-between text-center">
                ...
                <a href={item.checkoutUrl} target="_blank" rel="noopener noreferrer" className={`mt-6 block w-full text-center ${corBotao} text-white font-bold py-2.5 px-4 rounded-xl text-sm uppercase transition-colors`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {produtosDigitais.map((item, idx) => (
              <div key={idx} className="bg-white border border-neutral-200 rounded-2xl overflow-hidden shadow-sm p-4 flex flex-col justify-between relative">
                ...
                <a href={item.checkoutUrl} target="_blank" rel="noopener noreferrer" className={`mt-4 block w-full text-center ${corBotao} text-white font-bold py-2 px-4 rounded-xl text-xs uppercase transition-colors`}>
                  Adicionar ao carrinho
                </a>
              </div>
            ))}
