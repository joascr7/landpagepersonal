...
export default function Footer({ cliente }: FooterProps) {
  ...
  // Monta os links de contato dinamicamente com os dados do banco
  const mensagemWhats = encodeURIComponent(cliente.whatsapp_texto || "Olá! Gostaria de saber mais sobre os seus serviços.");
  const whatsappUrl = `https://wa.me/${cliente.whatsapp_numero}?text=${mensagemWhats}`;
  
  // Trata o link do Instagram para garantir que funcione mesmo se o cliente digitar apenas o @ no painel
  const usuarioInstagram = cliente.instagram_link?.replace("@", "").trim();
  const linkInstagram = `https://instagram.com/${usuarioInstagram}`;

  // Cores dinâmicas
  const corIconeVw = ehEstetica ? "group-hover:text-pink-500" : "group-hover:text-green-500";
  const corIconeG = ehEstetica ? "group-hover:text-pink-400" : "group-hover:text-red-400";
  const corSeloCirculo = ehEstetica ? "text-pink-500 bg-pink-500/10 border-pink-500/30" : "text-amber-500 bg-amber-500/10 border-amber-500/30";
    ...
    <footer className="bg-neutral-950 border-t border-neutral-900 text-neutral-400 py-12 md:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8 mb-12">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="text-sm flex items-center gap-2 hover:text-white text-neutral-400 transition-colors group">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16" className={`text-neutral-400 ${corIconeVw} transition-colors`}>
              ...
              {/* Gmail Dinâmico */}
              {cliente.gmail_contato && (
                <a href={`mailto:${cliente.gmail_contato}`} className="text-sm flex items-center gap-2 hover:text-white text-neutral-400 transition-colors group">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16" className={`text-neutral-400 ${corIconeG} transition-colors`}>
          ...
          {/* Coluna 2: Links Úteis Inteligentes */}
          <div>
            ...
            <ul className="space-y-2 text-sm font-medium">
              ...
              <li>
                <Link href={`/${cliente.slug}/planilhas-prontas`} className="hover:text-white transition-colors">
          ...
          {/* Coluna 3: Redes Sociais e Selo Dinâmico */}
          <div className="space-y-6">
            {cliente.instagram_link && (
              <div>
            ...
            <div className="inline-flex items-center gap-4 bg-neutral-900/50 border border-neutral-800 p-4 rounded-xl max-w-xs">
              <div className={`w-12 h-12 rounded-full ${corSeloCirculo} flex flex-col items-center justify-center font-mono select-none`}>
        ...
        <div className="border-t border-neutral-900 pt-8 text-center text-xs text-neutral-600">
          <p>© {new Date().getFullYear()} Studio {cliente.nome}. Todos os direitos reservados.</p>
        </div>
      </div>
    </footer>
  );
